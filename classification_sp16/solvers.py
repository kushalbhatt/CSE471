...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        w_new = []
        for i in range(len(param_vars)):
            vt_1 = self.momentum * vel_vars[i] - self.learning_rate * grad_tensors[i]
            vel_new.append((vel_vars[i],vt_1))
            w_new.append( (param_vars[i],param_vars[i]+vt_1) )

        return vel_new+w_new
        util.raiseNotDefined()
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            #0 - train data and 1-> target
            train_loss = session.run(loss_tensor,feed_dict={    placeholders[0]:next_batch[0],placeholders[1]:next_batch[1]})
            train_losses.append(train_loss)

            #update old_vars based on new_var_tensors
            session.run(update_ops,feed_dict={    placeholders[0]:next_batch[0],placeholders[1]:next_batch[1]})

            '''val_loss should be the loss of this iteration using all of the validation data'''
            next_batch = val_gen.next()
            val_loss = session.run(loss_tensor,feed_dict={placeholders[0]:next_batch[0],placeholders[1]:next_batch[1]})
            val_losses.append(val_loss)
